...
def plot_all_periods(df, out_dir):
    wd_nb = df[(df["day_type"] == "Weekday") & (df["direction"] == "N")]
...
def plot_worst_waits(df, out_dir):
    ...
    wd_nb_swap = df[(df["day_type"] == "Weekday") & (df["direction"] == "N") & swap_buckets]
    ...
    ax.set_title("Roosevelt Island — How Often Do Riders Face a Long Wait?\nNorthbound, Weekdays 6 AM–7 PM (Swap Active Hours)", fontsize=13, fontweight="bold", pad=12)
...
def write_talking_points(df, out_dir):
    ...
    am_b = get("Weekday","2:","N","Before swap"); am_a = get("Weekday","2:","N","After swap")
    md_b = get("Weekday","3:","N","Before swap"); md_a = get("Weekday","3:","N","After swap")
    ev_b_nb = get("Weekday","4:","N","Before swap"); ev_a_nb = get("Weekday","4:","N","After swap")
    ev_b_sb = get("Weekday","4:","S","Before swap"); ev_a_sb = get("Weekday","4:","S","After swap")
    swap_all = df[df["time_bucket"].str[:2].isin(SWAP_ACTIVE_BUCKETS) & (df["day_type"]=="Weekday") & (df["direction"]=="N")]
...
EVENING RUSH (4–7 PM) — The strongest finding:
  Northbound: {med(ev_b_nb)} min → {med(ev_a_nb)} min  ({chg(ev_b_nb, ev_a_nb)})
  Southbound: {med(ev_b_sb)} min → {med(ev_a_sb)} min  ({chg(ev_b_sb, ev_a_sb)})
  → Evening commute wait has more than doubled in both directions.

MORNING RUSH (6–9 AM):
  Northbound: {med(am_b)} min → {med(am_a)} min  ({chg(am_b, am_a)})
...
def main():
    ...
    # Load from results/ if it exists, otherwise fall back to current directory
    ...
    plot_all_periods(df, COMMUNITY_DIR)
    plot_evening_rush_spotlight(df, COMMUNITY_DIR)
    plot_worst_waits(df, COMMUNITY_DIR)
